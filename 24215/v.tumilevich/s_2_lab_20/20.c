...
void bubble_sort(AppContext* ctx, int direction) {
    int swapped;
    do {
        swapped = 0;
        ...
        while (curr->next != NULL && ctx->is_running) {
            ...
            int cmp_result = strcmp(curr->str, next_node->str);
            int should_swap = 0;
            
            if (direction > 0 && cmp_result > 0) {
                should_swap = 1;
            } else if (direction < 0 && cmp_result < 0) {
                should_swap = 1;
            }
...
void* sort_thread_func(void* arg) {
    ...
    while (1) {
        ...
        bubble_sort(td->ctx, td->direction);
    }
    return NULL;
}
...
int main() {
    ...
    for (int i = 0; i < NUM_SORT_THREADS; ++i) {
        ...
        thread_data[i].direction = (i == 0) ? 1 : -1;
