...
fn finalize_fetch(fidx: usize, success: bool, clients: *[types.MAX_CLIENTS]types.Client, fetches: *[types.MAX_FETCHES]types.Fetch, cache_state: *cache.Cache) void {
    var f = &fetches[fidx];
    if (!f.used) return;
    ...
    if (success and cache_idx >= 0) {
        cache.mark_complete(cache_state, cache_idx);
    }
    ...
    if (cache_idx >= 0) {
        for (0..clients.len) |cidx| {
            ...
            if (success) {
                const resp = cache.get_complete_response(cache_state, cache_idx) orelse {
        ...
        cache.release(cache_state, cache_idx);
    }
}

pub fn handle_fetch_event(fidx: usize, rev: c_short, clients: *[types.MAX_CLIENTS]types.Client, fetches: *[types.MAX_FETCHES]types.Fetch, cache_state: *cache.Cache) void {
    const f = &fetches[fidx];
    if (!f.used or f.fd < 0 or rev == 0) return;

    if (rev & (c.POLLERR | c.POLLNVAL) != 0) {
        finalize_fetch(fidx, false, clients, fetches, cache_state);
        return;
    }
    ...
    if (f.state == .FETCH_SENDING_REQUEST and f.request_data != null) {
        ...
        if (f.request_sent < f.request_len) {
            ...
            if (sent_raw > 0) {
                ...
                f.request_sent += sent;
            } else if (sent_raw < 0 and !connection.is_would_block(sent_raw)) {
                finalize_fetch(fidx, false, clients, fetches, cache_state);
                return;
            }
            if (f.request_sent >= f.request_len) {
    ...
    if (rev & c.POLLIN == 0) {
        if (rev & c.POLLHUP != 0) finalize_fetch(fidx, true, clients, fetches, cache_state);
        return;
    }
    ...
    if (n_raw > 0) {
        ...
        if (!cache.append_to(cache_state, f.cache_idx, buf[0..n])) finalize_fetch(fidx, false, clients, fetches, cache_state);
        return;
    }

    if (n_raw == 0) {
        finalize_fetch(fidx, true, clients, fetches, cache_state);
        return;
    }

    if (!connection.is_would_block(n_raw)) finalize_fetch(fidx, false, clients, fetches, cache_state);
}
