...
pub fn find_or_create(key: []const u8) i32 {
...
pub fn append_to(idx: i32, bytes: []const u8) bool {
...
pub fn mark_complete(idx: i32) void {
    if (idx < 0) return;
    const ce_idx: usize = @intCast(idx);
    var ce = &cache_entries[ce_idx];
    ce.complete = true;
