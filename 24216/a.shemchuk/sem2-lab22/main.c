...
pthread_mutex_t forks_mutex; // global mutex for fork acquisition
pthread_cond_t forks_cond;   // condition variable for waiting
...
void acquire_forks(int phil, int left, int right)
{
    ...
    while (1)
    {
        if (pthread_mutex_trylock(&forks[left]) == 0)
        {
            ...
            {
                safe_printf("Philosopher %d: got forks %d (left) and %d (right)\n", phil, left, right);
                break;
            }
            else
            {
        ...
        // if we reach here, couldn't get both forks
        // wait on condition variable
        pthread_cond_wait(&forks_cond, &forks_mutex);
        // after being woken up, loop again
    }
...
void down_forks(int f1, int f2)
{
    ...
    // signal all waiting philosophers (broadcast) because multiple may be able to proceed
