...
int last_word_length(editor_t *editor) {
    ...
    for (; start_index >= 0; start_index--) {
        if (isgraph(editor->chars[start_index])) {
            break;
        }
    }
    for (; start_index >= 0; start_index--) {
...
int process_print_char(editor_t *editor, char ch) {
...
int process_char(editor_t *editor, struct termios *attr, char ch) {
    if (ch == attr->c_cc[VERASE]) {
        ...
        return process_print_char(editor, ch);
    } else {
        const char bell = '\a';
...
int main() {
    ...
    while (true) {
        ...
        if (process_result == RESULT_ERROR) {
            is_error = true;
            break;
        }
        if (process_result == RESULT_END) {
            break;
        }
    }
