...
int main(int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		...
		char *owner;
		if (pwown == NULL) {
			snprintf(uid_buffer, 30, "%ju", (uintmax_t) statbuf.st_uid);
			owner = uid_buffer;
		} else {
			owner = pwown->pw_name;
		}
		...
		char *group;
		if (grp == NULL) {
			snprintf(gid_buffer, 30, "%ju", (uintmax_t) statbuf.st_gid);
			group = gid_buffer;
		} else {
			group = grp->gr_name;
		}
		...
		if (mode[0] == '-') {
			printf("%s  %-4lu  %-10s  %-10s  %-10jd  %.24s  %s\n",
					mode, (unsigned long) statbuf.st_nlink,
					owner, group,
					(intmax_t) statbuf.st_size, mtime, name
			);
		} else {
			printf("%s  %-4lu  %-10s  %-10s  %10s  %.24s  %s\n",
					mode, (unsigned long) statbuf.st_nlink,
					owner, group,
					"", mtime, name
			);
