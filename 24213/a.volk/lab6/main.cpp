...
struct file_reader_t {
  ...
  file_reader_t(int fd) : m_fd{fd}, m_buf{}, m_in_buf{0} {}

  file_reader_t(int fd, off_t seek) : file_reader_t{fd} {
    if (lseek(fd, seek, SEEK_SET) == (off_t)-1)
      throw fatal_error_t{strerror(errno)};
  }

  size_t load_buffer(off_t to_read) {
    ssize_t s =
        read(m_fd, m_buf.data(),
             to_read < (off_t)buffer_size ? (size_t)to_read : buffer_size);
    if (s == (ssize_t)-1)
      throw fatal_error_t{strerror(errno)};
  ...
  size_t load_buffer(void) { return load_buffer((off_t)buffer_size); }
...
bool wait_for_stdin() {
  ...
  int sel = select(1, &s, NULL, NULL, &tv) == 1;
...
ssize_t read_line_no() {
  ...
  file_reader_t stdin_reader{STDIN_FILENO};
  ...
  while ((std::cout << "you have 5 seconds to enter line no.\n"
    ...
    input_buf.reserve(MAX_INPUT_LEN + 1);

    auto buf_inserter = std::back_inserter(input_buf);

    size_t r = 0;

    while (stdin_reader.load_buffer() != 0) {
      r += stdin_reader.in_buffer();

      if (r > (size_t)MAX_INPUT_LEN) {
        std::cout << "input line too long" << std::endl;
        break;
      }

      std::copy(stdin_reader.get_buffer().begin(),
                stdin_reader.get_buffer().begin() + stdin_reader.in_buffer(),
                buf_inserter);

      if (stdin_reader.get_buffer()[stdin_reader.in_buffer() - 1] == '\n')
        break;
    }
    ...
    // make buffer content null terminated string
    input_buf.emplace_back('\0');

    std::istringstream input_stream{input_buf.data()};

    if (input_stream >> no) {
      char c_;
      if (input_stream >> c_) {
        std::cout << "warning! input past \"" << no << "\" ignored"
                  << std::endl;
      }
      ...
      std::cout << "could not recognize number" << std::endl;
    }
  }
...
void do_thing(int fd) {
  ...
  while ((line_no = read_line_no()) > 0) {
    ...
    while (rest > (off_t)0) {
      rest -= (off_t)reader.load_buffer(rest);
      const auto &buf = reader.get_buffer();
